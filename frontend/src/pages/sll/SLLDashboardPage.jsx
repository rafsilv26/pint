...
export default function SLLDashboardPage() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-ink">Painel de Controlo</h1>
          ...
          return (
            <Card key={s.label}>
              <div className="flex items-center justify-between">
                <div className={`grid h-11 w-11 place-items-center rounded-xl ${TINTS[s.tint] || TINTS.sky}`}><Icon size={20} /></div>
                {s.delta && <span className="flex items-center gap-0.5 text-xs font-semibold text-green-600"><ArrowUpRight size={12} /> {s.delta}</span>}
              </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          <h2 className="mb-4 font-semibold text-ink">Pontuação Mensal</h2>
          <div className="grid grid-cols-[2rem_1fr_auto_auto] gap-3 px-2 pb-2 text-xs font-medium text-muted">
            <span>#</span><span>Nome</span><span>Badges</span><span>Pontos</span>
          </div>
        ...
        <div className="space-y-6">
          <Card>
            <h2 className="mb-4 font-semibold text-ink">Badges Atribuídos</h2>
