...
//  Nota: alguns ecrãs do Figma têm mais informação do que o backend
//  devolve hoje — esses campos ficam com valores por omissão.
// =============================================================
...
export async function recuperarPassword() {
  // O backend ainda não expõe endpoint de recuperação de password.
...
function adaptBadge(b = {}) {
  return {
    ...
    requisitos: (b.requirements || b.requisitos || []).map((r) => r.titulo || r.descricao || r),
    slug: b.slug,
...
export async function getBadge(id) {
  return adaptBadge(await http(`/catalog/badges/${id}`))
}
...
export async function submeterCandidatura({ badgeId, descricao, ficheiros }) {
  ...
  // NOTA: para enviar os ficheiros reais é preciso anexar objetos File:
  //   ficheiros.forEach(f => form.append('evidencias', f.file))
  // Atualmente o ecrã guarda só o nome do ficheiro (mock); ajustar quando
  // o upload real estiver ligado.
  return http('/candidaturas', { method: 'POST', body: form, isForm: true })
}

// ---------- Badges conquistados ----------
export async function getMeusBadges() {
  const rows = await http('/catalog/consultor-badges').catch(() => [])
  return (rows || []).map((r) => ({
    badgeId: r.badgeId,
    nome: r.nome || `Badge #${r.badgeId}`,
    fornecedor: r.fornecedor || '',
    pontos: r.pointsObtained ?? 0,
...
export async function getEmailSignature() {
  ...
  return {
    nome: p.name || '',
    cargo: p.role || 'Consultor',
    email: p.email || '',
    telefone: '',
    localizacao: '',
    website: p.website || '',
    badges: res?.badges || [],
    templateHtml: res?.templateHtml || '',
  }
}
